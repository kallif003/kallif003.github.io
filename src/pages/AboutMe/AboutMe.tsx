import type { NextPage } from "next"
import React from "react"
import Header from "../../components/Header/Header"
import { H2Head, Body1, H3Head } from "../../components/Typography"
import foto from "../../../assets/Image/kallif.png"
import clickup from "../../../assets/Image/clickup.png"
import tailwind from "../../../assets/Image/tailwind.png"
import Footer from "../../components/Footer/Footer"
import {
	ItemAlignment,
	WidthAuto,
	TextH3,
	WorkToolsStyle,
	H3HeadStyle,
} from "../../components/PageStyles/styles"
import {
	AreaAboutMe,
	ImageAboutMe,
	AreaTextAboutMe,
	ContainerCards,
	Cards,
	Logo,
	WorkTools,
} from "../../components/Cards"

const AboutMe: NextPage = () => {
	return (
		<div className="text-white">
			<Header home="/HomePage" project="/HomePage#projetos" />

			<div className="pt-40 ">
				<AreaAboutMe className="mx-auto" data-cy="div-aboutMe">
					<ImageAboutMe
						src={foto}
						alt="Me"
						width={300}
						height={326}
						id="imgData"
					/>
					<AreaTextAboutMe>
						<H2Head>Sobre Mim </H2Head>
						<Body1>
							Me chamo Kallif Abdon Abrahão, tenho 32 anos, sou pai de uma
							menina linda e atuo profissionalmente como desenvolvedor
							front-end.
						</Body1>
						<Body1>
							Aqui nesse portfólio você pode conhecer mais sobre mim, sobre as
							tecnologias que domino, sobre meus projetos e experiências.
						</Body1>
						<Body1>
							Você pode entrar em contato comigo pelo Whatsaap, email ou
							Linkedin e pode aproveitar também pra conhecer meu Github.
						</Body1>
					</AreaTextAboutMe>
				</AreaAboutMe>
				<h1 id="skills" />
				<H3Head className={`${TextH3} ${H3HeadStyle} `}>
					Minhas Habilidades
				</H3Head>

				<div data-cy="div-minhas-habilidades">
					<ContainerCards className={`${WidthAuto} sm:mt-10 mt-20`}>
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo
								src="data:image/svg+xml;base64,"
								alt="Js"
								width={70}
								height={72}
							/>
							<H3Head>Javascript</H3Head>
						</Cards>
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo
								src="data:image/svg+xml;base64,"
								alt="React/Next"
								width={70}
								height={72}
							/>
							<H3Head>React/Next</H3Head>
						</Cards>
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo
								src="data:image/svg+xml;base64,"
								alt="React Native"
								width={70}
								height={72}
							/>
							<H3Head>React Native</H3Head>
						</Cards>
					</ContainerCards>

					<ContainerCards className={`${WidthAuto} sm:mt-10 mt-20 `}>
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo
								src="data:image/png;base64,"
								alt="Js"
								width={70}
								height={72}
							/>
							<H3Head>Html/Css</H3Head>
						</Cards>
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo
								src="data:image/png;base64,"
								alt="React Native"
								width={70}
								height={72}
							/>
							<H3Head>Jest/Cypress</H3Head>
						</Cards>
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo src={tailwind} alt="React/Next" width={60} height={60} />
							<H3Head>Tailwind</H3Head>
						</Cards>
					</ContainerCards>

					<H3Head className={`${TextH3} mb-20 ${H3HeadStyle}`}>
						Experiência de Trabalho
					</H3Head>

					<ContainerCards className="mb-28">
						<Cards className={`${ItemAlignment} flex-col`}>
							<Logo
								src="data:image/png;base64,"
								alt="Lipe.Dev"
								width={70}
								height={72}
								className="pb-5"
							/>
							<H3Head className="pt-2 text-yellow">Lipe.Dev</H3Head>
							<Body1 className="pt-2 text-center">
								Tralhando como front-end, desenvolvendo projetos web e mobile
							</Body1>
						</Cards>
					</ContainerCards>
				</div>

				<H3Head className={`${TextH3} ${H3HeadStyle}`}>
					Ferramentas de Trabalho
				</H3Head>

				<ContainerCards
					className={`${WidthAuto} sm:mt-10 mt-20 mb-44 sm:mb-20`}
					data-cy="div-workTools">
					<WorkTools className={`${ItemAlignment} ${WorkToolsStyle}`}>
						<Logo
							src={clickup}
							alt="clickup"
							width={90}
							height={82}
							className="pb-5"
						/>
						<H3Head className="pt-1 text-yellow">Clickup</H3Head>
					</WorkTools>
					<WorkTools className={`${ItemAlignment} ${WorkToolsStyle}`}>
						<Logo
							src="data:image/png;base64,"
							alt="Lipe.Dev"
							width={70}
							height={72}
							className="pb-5"
						/>
						<H3Head className="pt-3 text-yellow">GitHub</H3Head>
					</WorkTools>
					<WorkTools className={`${ItemAlignment} ${WorkToolsStyle}`}>
						<Logo
							src="data:image/png;base64,"
							alt="slack"
							width={70}
							height={72}
							className="pb-5"
						/>
						<H3Head className="pt-3 text-yellow">Slack</H3Head>
					</WorkTools>
				</ContainerCards>
			</div>
			<Footer />
		</div>
	)
}

export default AboutMe
